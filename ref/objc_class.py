...
class ObjcClass:
    ...
    def dumptofile(self, fname=None):
        if not fname:
            fname = idc.GetInputFile() + '.cls'
        else:
            fname = fname
        ...
        if not self.class_info.has_key('name'):
            return
        content = list()
        content.append('\r\nClass: %s' % self.class_info['name'])
        content.append('Attributes:')
        content.append('  IsTopClass: %d' % self.class_info['top_class'])
        content.append('  Instance Size: %d' % self.class_info['instance_size'])
        content.append('  Methods: %d' % len(self.class_info['methods']))
        content.append('  Protocols: %d' % len(self.class_info['protocols']))
        content.append('  Instance Vars: %d' % len(self.class_info['ivars']))
        content.append('  properties: %d' % len(self.class_info['properties']))

        if len(self.class_info['methods']):
            content.append('Method list:\n  %s' % repr(self.class_info['methods']).replace('\n', '\n  '))
